...
#include <mcmc/stats/seed_values.hpp>

// [MCMC_BEGIN]
mcmclib_inline bool mcmc::internal::aees_impl(
  ...
  const size_t n_total_draws =
      n_keep_draws + K * (n_initial_draws + n_burnin_draws);

  //

  const fp_t par_scale = settings.aees_settings.par_scale;
  const Mat_t cov_mcmc =
      (BMO_MATOPS_SIZE(settings.aees_settings.cov_mat) == n_vals * n_vals)
          ? settings.aees_settings.cov_mat
  ...
  std::function<fp_t(const ColVec_t &vals_inp, void *target_data)>
      box_log_kernel =
          [target_log_kernel, vals_bound, bounds_type, lower_bounds,
           upper_bounds](const ColVec_t &vals_inp, void *target_data) -> fp_t {
    if (vals_bound) {
      ColVec_t vals_inv_trans =
          inv_transform(vals_inp, bounds_type, lower_bounds, upper_bounds);

      return target_log_kernel(vals_inv_trans, target_data) +
  ...
  Mat_t kernel_vals(K,
                    n_total_draws); // target kernel values at temperature = 1
  Mat_t kernel_vals_prev(
      2,
      K); // store kernel values from run (n - 1) at two different temperatures
  Mat_t kernel_vals_new(
      2,
      K); // store kernel values from run (n)     at two different temperatures
  ...
  for (size_t draw_ind = 0; draw_ind < n_total_draws; ++draw_ind) {
    ...
    X_new.col(0) =
        single_step_mh(X_prev.col(0), temper_vec(0), prop_scaling_mat,
                       box_log_kernel, target_data, rand_engine, &val_out_hot);
...
#pragma omp parallel for num_threads(omp_n_threads)
    ...
    for (size_t temper_ind = 1; temper_ind < K; ++temper_ind) {
      ...
      if (draw_ind > temper_ind * (n_initial_draws + n_burnin_draws)) {
        ...
        const fp_t z_eps =
            bmo::stats::runif<fp_t>(rand_engines_vec[thread_num]);

        if (z_eps > ee_prob_par) {
          X_new.col(temper_ind) =
              single_step_mh(X_prev.col(temper_ind), temper_vec(temper_ind),
                             prop_scaling_mat, box_log_kernel, target_data,
                             rand_engines_vec[thread_num], &val_out_j);

          kernel_vals_new(0, temper_ind) =
              val_out_j / temper_vec(temper_ind - 1);
          kernel_vals_new(1, temper_ind) = val_out_j / temper_vec(temper_ind);
        } else {
          size_t draws_j_begin_ind =
              (temper_ind - 1) * (n_initial_draws + n_burnin_draws);

          size_t ring_ind_spacing = std::floor(
              static_cast<fp_t>(draw_ind - draws_j_begin_ind + 1) / n_rings);
          ...
          if (ring_ind_spacing == 0) {
            ...
            // ColVec_t past_kernel_vals =
            // arma::trans(kernel_vals(arma::span(temper_ind-1,temper_ind-1),arma::span(draws_j_begin_ind,
            // draw_ind)));
            ColVec_t past_kernel_vals = BMO_MATOPS_TRANSPOSE(
                BMO_MATOPS_MIDDLE_COLS(kernel_vals.row(temper_ind - 1),
                                       draws_j_begin_ind, draw_ind));
            ...
            for (size_t i = 0; i < (n_rings - 1); ++i) {
              ...
              ring_vals(temper_ind - 1, i) =
                  (past_kernel_vals(ring_i_ind) +
                   past_kernel_vals(ring_i_ind - 1)) /
                  fp_t(2);
            }

            size_t which_ring = 0;

            while (which_ring < (n_rings - 1) &&
                   kernel_vals(temper_ind, draw_ind - 1) >
                       ring_vals(temper_ind - 1, which_ring)) {
              which_ring++;
            }
            ...
            const size_t ind_mix = sort_vec(
                static_cast<size_t>(ring_ind_spacing * which_ring +
                                    std::floor(z_tmp * ring_ind_spacing)));

            //

            X_new.col(temper_ind) =
                draw_storage.mat(ind_mix).col(temper_ind - 1);
            ...
            const fp_t comp_val_1 = kernel_vals_new(1, temper_ind) -
                                    kernel_vals_prev(1, temper_ind);
            const fp_t comp_val_2 = kernel_vals_prev(0, temper_ind) -
                                    kernel_vals_new(0, temper_ind);
            ...
            if (z > std::exp(comp_val)) {
              X_new.col(temper_ind) = X_prev.col(temper_ind);
              kernel_vals_new.col(temper_ind) =
                  kernel_vals_prev.col(temper_ind);
            }
          }
        }

        // store target kernel values
        kernel_vals(temper_ind, draw_ind) = box_log_kernel(
            X_new.col(temper_ind), target_data); // temperature = 1
      }
    }
  ...
  for (size_t draw_ind = draw_ind_begin; draw_ind < n_total_draws; ++draw_ind) {
    // ColVec_t tmp_vec =
    // draw_storage(arma::span(),arma::span(K-1,K-1),arma::span(i,i));
...
mcmclib_inline bool
mcmc::aees(const ColVec_t &initial_vals,
           std::function<fp_t(const ColVec_t &vals_inp, void *target_data)>
               target_log_kernel,
           Mat_t &draws_out, void *target_data) {
  return internal::aees_impl(initial_vals, target_log_kernel, draws_out,
                             target_data, nullptr);
}
