...
#include <mcmc/de.hpp>
#include <mcmc/misc/bounds_check.hpp>
...
// [MCMC_BEGIN]
mcmclib_inline bool internal::de_impl(
    const ColVec_t &initial_vals,
    std::function<fp_t(const ColVec_t &vals_inp, void *target_data)>
        target_log_kernel,
    Cube_t &draws_out, void *target_data, algo_settings_t *settings_inp) {
  bool success = false;
  ...
  ColVec_t par_initial_lb =
      (BMO_MATOPS_SIZE(settings.de_settings.initial_lb) == n_vals)
          ? settings.de_settings.initial_lb
          : BMO_MATOPS_ARRAY_ADD_SCALAR(initial_vals, -0.5);
  ColVec_t par_initial_ub =
      (BMO_MATOPS_SIZE(settings.de_settings.initial_ub) == n_vals)
          ? settings.de_settings.initial_ub
          : BMO_MATOPS_ARRAY_ADD_SCALAR(initial_vals, 0.5);

  sampling_bounds_check(vals_bound, n_vals, bounds_type, lower_bounds,
                        upper_bounds, par_initial_lb, par_initial_ub);
...
#pragma omp parallel for num_threads(omp_n_threads) firstprivate(rand_vec)
  ...
  for (size_t i = 0; i < n_pop; ++i) {
    ...
    bmo::stats::internal::runif_vec_inplace<fp_t>(
        n_vals, rand_engines_vec[thread_num], rand_vec);

    X.row(i) = BMO_MATOPS_TRANSPOSE(
        par_initial_lb +
        BMO_MATOPS_HADAMARD_PROD((par_initial_ub - par_initial_lb), rand_vec));

    fp_t prop_kernel_val =
        box_log_kernel(BMO_MATOPS_TRANSPOSE(X.row(i)), target_data);
...
#pragma omp parallel for num_threads(omp_n_threads) firstprivate(rand_vec)
    ...
    for (size_t i = 0; i < n_pop; ++i) {
      ...
      // arma::rowvec prop_rand = arma::randu(1,n_vals)*(2*par_b) - par_b; //
      // generate a vector of U[-b,b] RVs
      bmo::stats::internal::runif_vec_inplace(
          n_vals, -par_b, par_b, rand_engines_vec[thread_num], rand_vec);

      RowVec_t X_prop =
          X.row(i) +
          BMO_MATOPS_ARRAY_MULT_SCALAR(X.row(c_1) - X.row(c_2), par_gamma_run) +
          BMO_MATOPS_TRANSPOSE(rand_vec);

      fp_t prop_kernel_val =
          box_log_kernel(BMO_MATOPS_TRANSPOSE(X_prop), target_data);
      ...
      if (comp_val > temperature_j * std::log(z)) {
        ...
        if (draw_ind >= n_burnin_draws) {
          // n_accept++; // this would lead to a race condition if parallel
          // samlping were used
...
#pragma omp parallel for num_threads(omp_n_threads)
    ...
    for (size_t draw_ind = 0; draw_ind < n_keep_draws; draw_ind++) {
      for (size_t jj = 0; jj < n_pop; jj++) {
        draws_out.mat(draw_ind).row(jj) =
            inv_transform<RowVec_t>(draws_out.mat(draw_ind).row(jj),
                                    bounds_type, lower_bounds, upper_bounds);
      }
    }
  }
...
mcmclib_inline bool
de(const ColVec_t &initial_vals,
   std::function<fp_t(const ColVec_t &vals_inp, void *target_data)>
       target_log_kernel,
   Cube_t &draws_out, void *target_data) {
  return internal::de_impl(initial_vals, target_log_kernel, draws_out,
                           target_data, nullptr);
}
...
de(const ColVec_t &initial_vals,
   std::function<fp_t(const ColVec_t &vals_inp, void *target_data)>
       target_log_kernel,
   Cube_t &draws_out, void *target_data, algo_settings_t &settings) {
  return internal::de_impl(initial_vals, target_log_kernel, draws_out,
