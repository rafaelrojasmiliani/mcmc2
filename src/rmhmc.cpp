...
// [MCMC_BEGIN]
mcmclib_inline bool internal::rmhmc_impl(
    const ColVec_t &initial_vals,
    std::function<fp_t(const ColVec_t &vals_inp, ColVec_t *grad_out,
                       void *target_data)>
        target_log_kernel,
    std::function<Mat_t(const ColVec_t &vals_inp, Cube_t *tensor_deriv_out,
                        void *tensor_data)>
        tensor_fn,
    Mat_t &draws_out, void *target_data, void *tensor_data,
  ...
  std::function<fp_t(const ColVec_t &vals_inp, ColVec_t *grad_out,
                     void *box_data)>
      box_log_kernel = [target_log_kernel, vals_bound, bounds_type,
                        lower_bounds, upper_bounds](const ColVec_t &vals_inp,
                                                    ColVec_t *grad_out,
    ...
    if (vals_bound) {
      ...
      for (size_t i = 0; i < n_vals; ++i) {
        ...
        grad_obj(i) =
            -grad_obj(i) +
            fp_t(0.5) *
                (BMO_MATOPS_TRACE(tmp_mat) -
                 BMO_MATOPS_DOT_PROD(BMO_MATOPS_TRANSPOSE(tmp_mat) * mntm_inp,
                                     inv_tensor_mat * mntm_inp));
      }
  ...
  std::function<Mat_t(const ColVec_t &vals_inp, Cube_t *tensor_deriv_out,
                      void *tensor_data)>
      box_tensor_fn = [tensor_fn, vals_bound, bounds_type, lower_bounds,
                       upper_bounds](const ColVec_t &vals_inp,
                                     Cube_t *tensor_deriv_out,
                                     void *tensor_data) -> Mat_t {
    if (vals_bound) {
      ColVec_t vals_inv_trans =
          inv_transform(vals_inp, bounds_type, lower_bounds, upper_bounds);
  ...
  fp_t prev_U = cons_term - box_log_kernel(first_draw, nullptr, target_data) +
                fp_t(0.5) * BMO_MATOPS_LOG_DET(new_tensor);
  fp_t prop_U = prev_U;
  ...
  for (size_t draw_ind = 0; draw_ind < n_total_draws; draw_ind++) {
    ...
    prev_K =
        BMO_MATOPS_DOT_PROD(new_mntm, inv_prev_tensor * new_mntm) / fp_t(2);
    ...
    for (size_t k = 0; k < n_leap_steps; ++k) {
      ...
      for (size_t kk = 0; kk < n_fp_steps; ++kk) {
        prop_mntm =
            new_mntm + mntm_update_fn(new_draw, prop_mntm, target_data,
                                      step_size, inv_prev_tensor,
                                      prev_deriv_cube, nullptr); // half-step
      }
      ...
      for (size_t kk = 0; kk < n_fp_steps; ++kk) {
        inv_new_tensor =
            BMO_MATOPS_INV(box_tensor_fn(prop_draw, nullptr, tensor_data));

        prop_draw = new_draw + fp_t(0.5) * step_size *
      ...
      new_mntm +=
          mntm_update_fn(new_draw, new_mntm, target_data, step_size,
                         inv_new_tensor, new_deriv_cube, nullptr); // half-step
    }

    prop_U = cons_term - box_log_kernel(new_draw, nullptr, target_data) +
             fp_t(0.5) * BMO_MATOPS_LOG_DET(new_tensor);
    ...
    const fp_t comp_val =
        std::min(fp_t(0.01), -(prop_U + prop_K) + (prev_U + prev_K));
...
mcmclib_inline bool
rmhmc(const ColVec_t &initial_vals,
      std::function<fp_t(const ColVec_t &vals_inp, ColVec_t *grad_out,
                         void *target_data)>
          target_log_kernel,
      std::function<Mat_t(const ColVec_t &vals_inp, Cube_t *tensor_deriv_out,
          ...
          tensor_fn,
      Mat_t &draws_out, void *target_data, void *tensor_data) {
  return internal::rmhmc_impl(initial_vals, target_log_kernel, tensor_fn,
                              draws_out, target_data, tensor_data, nullptr);
}
...
rmhmc(const ColVec_t &initial_vals,
      ...
      algo_settings_t &settings) {
  return internal::rmhmc_impl(initial_vals, target_log_kernel, tensor_fn,
                              draws_out, target_data, tensor_data, &settings);
}
} // namespace mcmc
