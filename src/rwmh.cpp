...
namespace mcmc {
// [MCMC_BEGIN]
mcmclib_inline bool internal::rwmh_impl(
  ...
  const Mat_t cov_mcmc =
      (BMO_MATOPS_SIZE(settings.rwmh_settings.cov_mat) == n_vals * n_vals)
  ...
  for (size_t draw_ind = 0; draw_ind < n_total_draws; ++draw_ind) {
    ...
    // new_draw = prev_draw + cov_mcmc_chol *
    // bmo::stats::rnorm_vec<fp_t>(n_vals, rand_engine);
...
mcmclib_inline bool
rwmh(const ColVec_t &initial_vals,
     std::function<fp_t(const ColVec_t &vals_inp, void *target_data)>
         target_log_kernel,
     Mat_t &draws_out, void *target_data) {
  return internal::rwmh_impl(initial_vals, target_log_kernel, draws_out,
                             target_data, nullptr);
}
