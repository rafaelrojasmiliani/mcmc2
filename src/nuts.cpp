...
// [MCMC_BEGIN]
mcmclib_inline bool
internal::nuts_impl(const ColVec_t &initial_vals,
  ...
  const size_t n_adapt_draws =
      (settings.nuts_settings.n_adapt_draws <= n_total_draws)
          ? settings.nuts_settings.n_adapt_draws
          : n_total_draws;
  ...
  const Mat_t precond_matrix =
      (BMO_MATOPS_SIZE(settings.nuts_settings.precond_mat) == n_vals * n_vals)
          ? settings.nuts_settings.precond_mat
          : BMO_MATOPS_EYE(n_vals);
  ...
  const ColVecInt_t bounds_type =
      determine_bounds_type(vals_bound, n_vals, lower_bounds, upper_bounds);
  ...
  if (settings.nuts_settings.omp_n_threads > 0) {
    ...
    omp_n_threads =
        std::max(1, static_cast<int>(omp_get_max_threads()) /
                        2); // OpenMP often detects the number of
                            // virtual/logical cores, not physical cores
  }
#endif
  ...
  std::function<fp_t(const ColVec_t &vals_inp, ColVec_t *grad_out,
                     void *box_data)>
      box_log_kernel_fn =
          [target_log_kernel, vals_bound, bounds_type, lower_bounds,
           upper_bounds](const ColVec_t &vals_inp, ColVec_t *grad_out,
                         void *target_data) -> fp_t {
    if (vals_bound) {
      ColVec_t vals_inv_trans =
          inv_transform(vals_inp, bounds_type, lower_bounds, upper_bounds);

      return target_log_kernel(vals_inv_trans, nullptr, target_data) +
             log_jacobian(vals_inp, bounds_type, lower_bounds, upper_bounds);
    } else {
  ...
  std::function<ColVec_t(const ColVec_t &pos_inp, const ColVec_t &mntm_inp,
                         void *target_data, const fp_t step_size,
                         Mat_t *jacob_matrix_out)>
      mntm_update_fn =
          [target_log_kernel, vals_bound, bounds_type, lower_bounds,
           upper_bounds](const ColVec_t &pos_inp, const ColVec_t &mntm_inp,
                         void *target_data, const fp_t step_size,
                         Mat_t *jacob_matrix_out) -> ColVec_t {
    const size_t n_vals = BMO_MATOPS_SIZE(pos_inp);

    ColVec_t grad_obj(n_vals);

    if (vals_bound) {
      ColVec_t pos_inv_trans =
          inv_transform(pos_inp, bounds_type, lower_bounds, upper_bounds);
      ...
      Mat_t jacob_matrix =
          inv_jacobian_adjust(pos_inp, bounds_type, lower_bounds, upper_bounds);
  ...
  std::function<void(const fp_t step_size, const size_t n_leap_steps,
                     ColVec_t &new_draw, ColVec_t &new_mntm, void *target_data)>
      leap_frog_fn = [mntm_update_fn, inv_precond_matrix](
                         const fp_t step_size, const size_t n_leap_steps,
                         ColVec_t &new_draw, ColVec_t &new_mntm,
                         void *target_data) -> void {
    for (size_t k = 0; k < n_leap_steps; ++k) {
      new_mntm = mntm_update_fn(new_draw, new_mntm, target_data, step_size,
                                nullptr); // first half-step
  ...
  if (vals_bound) { // should we transform the parameters?
    first_draw =
        transform(initial_vals, bounds_type, lower_bounds, upper_bounds);
  }
  ...
  fp_t step_size =
      nuts_find_initial_step_size(first_draw, mntm_vec, inv_precond_matrix,
                                  box_log_kernel_fn, leap_frog_fn, target_data);
  ...
  for (size_t draw_ind = 0; draw_ind < n_total_draws; ++draw_ind) {
    bmo::stats::internal::rnorm_vec_inplace<fp_t>(n_vals, rand_engine,
                                                  rand_vec);
    ...
    log_rand_val =
        std::log(bmo::stats::runif<fp_t>(rand_engine)) - prev_U - prev_K;
    ...
    while (s_val == size_t(1) && tree_depth < max_tree_depth) {
      ...
      if (direction_val == -1) {
        ...
        nuts_build_tree(
            direction_val, step_size, log_rand_val, prev_U, prev_K, prev_draw,
            mntm_vec, inv_precond_matrix, box_log_kernel_fn, leap_frog_fn,
            tree_depth, new_draw, dummy_draw, draw_neg, dummy_mntm, mntm_neg,
        ...
        nuts_build_tree(
            direction_val, step_size, log_rand_val, prev_U, prev_K, prev_draw,
            mntm_vec, inv_precond_matrix, box_log_kernel_fn, leap_frog_fn,
            tree_depth, new_draw, draw_pos, dummy_draw, mntm_pos, dummy_mntm,
            n_p_val, s_p_val, alpha_val, n_alpha_val, rand_engine, target_data);
      }
      ...
      int check_val_1 =
          BMO_MATOPS_DOT_PROD(draw_pos - draw_neg, mntm_neg) >= fp_t(0);
      int check_val_2 =
          BMO_MATOPS_DOT_PROD(draw_pos - draw_neg, mntm_pos) >= fp_t(0);
    ...
    if (draw_ind < n_adapt_draws) {
      h_val +=
          (1 / fp_t(draw_ind + 1 + t0_val)) *
          (target_accept_rate - (fp_t(alpha_val) / fp_t(n_alpha_val)) - h_val);

      step_size =
          std::exp(mu_val - h_val * std::sqrt(draw_ind + 1) / gamma_val);

      epsilon_bar *= std::exp(std::pow(draw_ind + 1, -kappa_val) *
                              (std::log(step_size) - std::log(epsilon_bar)));
    } else {
    ...
    if (draw_ind >= n_burnin_draws) {
      draws_out.row(draw_ind - n_burnin_draws) =
          BMO_MATOPS_TRANSPOSE(prev_draw);
      n_accept += good_round;
    }
  }
...
#pragma omp parallel for num_threads(omp_n_threads)
    ...
    for (size_t draw_ind = 0; draw_ind < n_keep_draws; ++draw_ind) {
      draws_out.row(draw_ind) = inv_transform<RowVec_t>(
          draws_out.row(draw_ind), bounds_type, lower_bounds, upper_bounds);
    }
  }
...
mcmclib_inline bool
nuts(const ColVec_t &initial_vals,
     std::function<fp_t(const ColVec_t &vals_inp, ColVec_t *grad_out,
                        void *target_data)>
         target_log_kernel,
     Mat_t &draws_out, void *target_data) {
  return internal::nuts_impl(initial_vals, target_log_kernel, draws_out,
                             target_data, nullptr);
}
