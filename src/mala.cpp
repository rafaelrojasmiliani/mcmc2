...
// [MCMC_BEGIN]
mcmclib_inline bool
internal::mala_impl(const ColVec_t &initial_vals,
  ...
  const Mat_t precond_matrix =
      (BMO_MATOPS_SIZE(settings.mala_settings.precond_mat) == n_vals * n_vals)
          ? settings.mala_settings.precond_mat
  ...
  std::function<ColVec_t(const ColVec_t &vals_inp, void *target_data,
      ...
      mala_mean_fn = [target_log_kernel, vals_bound, bounds_type, lower_bounds,
                      upper_bounds,
                      precond_matrix](const ColVec_t &vals_inp,
    ...
    if (vals_bound) {
      ...
      Mat_t jacob_matrix = inv_jacobian_adjust(vals_inp, bounds_type,
      ...
      return vals_inp + step_size * step_size * jacob_matrix * precond_matrix *
                            grad_obj / fp_t(2);
    } else {
      target_log_kernel(vals_inp, &grad_obj, target_data);

      return vals_inp +
             step_size * step_size * precond_matrix * grad_obj / fp_t(2);
    }
  };
  ...
  for (size_t draw_ind = 0; draw_ind < n_total_draws; ++draw_ind) {
    ...
    if (vals_bound) {
      ...
      new_draw = mean_vec + step_size * BMO_MATOPS_CHOL_LOWER(jacob_matrix) *
                                sqrt_precond_matrix * rand_vec;
    } else {
      new_draw = mala_mean_fn(prev_draw, target_data, step_size, nullptr) +
                 step_size * sqrt_precond_matrix * rand_vec;
    }
    ...
    const fp_t comp_val = std::min(
        fp_t(0.01),
        prop_LP - prev_LP +
            mala_prop_adjustment(new_draw, prev_draw, step_size, vals_bound,
                                 precond_matrix, mala_mean_fn, target_data));
...
mcmclib_inline bool
mala(const ColVec_t &initial_vals,
     std::function<fp_t(const ColVec_t &vals_inp, ColVec_t *grad_out,
                        void *target_data)>
         target_log_kernel,
     Mat_t &draws_out, void *target_data) {
  return internal::mala_impl(initial_vals, target_log_kernel, draws_out,
                             target_data, nullptr);
}
