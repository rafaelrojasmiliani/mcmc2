...
/*
 * Sampling from a Gaussian Mixture Distribution using the Adaptive Equi-Energy
 * Sampler (AEES)
 *
...
// -I$EIGEN_INCLUDE_PATH -I./../../include/ aees_mixture.cpp -o aees_mixture.out
// -L./../.. -lmcmc
...
double gaussian_mixture(const Eigen::VectorXd &X_vec_inp,
                        const Eigen::VectorXd &weights,
                        const Eigen::MatrixXd &mu,
                        const Eigen::VectorXd &sig_sq) {
  ...
  for (int i = 0; i < n_mix; ++i) {
    ...
    dens_val +=
        weights(i) * std::exp(-0.5 * dist_val / sig_sq(i)) /
        std::pow(2.0 * pi * sig_sq(i), static_cast<double>(n_vals) / 2.0);
  }
...
double target_log_kernel(const Eigen::VectorXd &vals_inp, void *target_data) {
...
int main() {
  ...
  settings.aees_settings.cov_mat =
      0.35 * Eigen::MatrixXd::Identity(n_vals, n_vals);
  ...
  Eigen::Matrix<bool, Eigen::Dynamic, Eigen::Dynamic> pos_inds =
      (draws_out.array() > 0.1);

  Eigen::VectorXd mean_vec = Eigen::VectorXd::Zero(2);

  for (int i = 0; i < n_vals; ++i) {
    for (size_t draw_ind = 0; draw_ind < settings.aees_settings.n_keep_draws;
         ++draw_ind) {
  ...
  Eigen::Matrix<bool, Eigen::Dynamic, Eigen::Dynamic> neg_inds =
      (draws_out.array() < -0.1);
