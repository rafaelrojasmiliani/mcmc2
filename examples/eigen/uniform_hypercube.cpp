...
// $CXX -Wall -std=c++14 -O3 -march=native -ffp-contract=fast
// -I$EIGEN_INCLUDE_PATH -I./../../include/ \
...
std::optional<double> log_hyperrectangle_density(const Eigen::VectorXd &values,
                                                 const Bounds &bounds) {
...
double log_hyperrectangle_density_adapter(const Eigen::VectorXd &values,
                                          void *data) {
  if (data == nullptr) {
...
//! Draw iid samples from the uniform distribution on the supplied
//! hyperrectangle.
//!
...
Eigen::MatrixXd draw_uniform_samples(std::mt19937 &rng, std::size_t n_samples,
                                     const Bounds &bounds) {
  ...
  for (std::size_t i = 0; i < n_samples; ++i) {
    for (int j = 0; j < dimension; ++j) {
      samples(static_cast<Eigen::Index>(i), j) =
          per_dimension_distributions[static_cast<std::size_t>(j)](rng);
    }
  }
...
Eigen::VectorXd compute_column_variances(const Eigen::MatrixXd &samples,
  ...
  variances = (centered.array().square().colwise().sum() /
...
int main() {
  ...
  //   P(|\bar{X}_n - \mathbb{E}[X]| \geq \epsilon) \leq
  //   2\exp\left(-\frac{2n\epsilon^2}{(b_i - a_i)^2}\right).
  // Solving for n shows that drawing at least
  //   n \geq \frac{(b_i - a_i)^2}{2\epsilon^2}
  ...
  const std::size_t n_samples = std::max<std::size_t>(
      1, static_cast<std::size_t>(
             std::ceil((max_width * max_width) * std::log(2.0 / delta) /
                       (2.0 * epsilon * epsilon))));
  ...
  const std::size_t n_burnin = std::max<std::size_t>(
      static_cast<std::size_t>(dimension * 200), n_samples / 10);
  ...
  //   * &bounds makes the dimension-specific bounds available to the
  //   log-density.
  //   * settings carries all tuning choices discussed above.
  mcmc::rwmh(initial_values, log_hyperrectangle_density_adapter, mcmc_draws,
             static_cast<void *>(&bounds), settings);
  ...
  const Eigen::VectorXd mcmc_variance =
      compute_column_variances(mcmc_draws, mcmc_mean);
  const Eigen::VectorXd direct_variance =
      compute_column_variances(direct_draws, direct_mean);

  const double mean_difference =
      (mcmc_mean - direct_mean).cwiseAbs().maxCoeff();
  const double variance_difference =
      (mcmc_variance - direct_variance).cwiseAbs().maxCoeff();
  ...
  std::cout << "  MCMC acceptance rate: "
            << static_cast<double>(settings.rwmh_settings.n_accept_draws) /
                   static_cast<double>(settings.rwmh_settings.n_keep_draws)
            << '\n';
  ...
  if (mean_difference > mean_tolerance ||
    ...
    std::cerr
        << "Consistency check failed: sample summaries differ beyond tolerance."
        << std::endl;
    return EXIT_FAILURE;
  }
