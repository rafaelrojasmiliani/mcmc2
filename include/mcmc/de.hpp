 ...
 * @param draws_out a 3-dimensional array posterior draws, where each matrix
 * represents one draw.
 * @param target_data additional data passed to the user-provided function.
 *
 * @return a boolean value indicating successful completion of the sampling
 * algorithm.
 */

bool de(const ColVec_t &initial_vals,
...
bool de_impl(const ColVec_t &initial_vals,
             std::function<fp_t(const ColVec_t &vals_inp, void *target_data)>
                 target_log_kernel,
             Cube_t &draws_out, void *target_data,
             algo_settings_t *settings_inp);

inline fp_t de_cooling_schedule(const size_t s, const size_t n_gen) {
  return fp_t(1);
}
