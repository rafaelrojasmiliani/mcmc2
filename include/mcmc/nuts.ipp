...
inline fp_t nuts_find_initial_step_size(
    const ColVec_t &draw_vec, const ColVec_t &mntm_vec,
  ...
  while (check_cond) {
    ...
    a_val = 2 * ((-(prop_U + prop_K) + (prev_U + prev_K)) > std::log(0.5)) -
            1; // not obvious from the paper, but I think we should update a...
...
inline void nuts_build_tree(
    ...
    rand_engine_t &rand_engine, void *target_data) {
  const fp_t max_tuning_par =
      1000; // from the paper: We recommend setting \delta_max to a large value
            // like 1000 so that it does not interfere with the algorithm so
            // long as the simulation is even moderately accurate.
  ...
  if (tree_depth == size_t(0)) {
    ...
    alpha_val =
        std::exp(std::min(fp_t(0), -(prop_U + prop_K) + (prev_U + prev_K)));
    n_alpha_val = 1;
  } else {
    size_t n_p_val;
    size_t s_p_val;
    ...
    nuts_build_tree(direction_val, step_size, log_rand_val, prev_U, prev_K,
                    draw_vec, mntm_vec, inv_precond_matrix, box_log_kernel_fn,
                    leap_frog_fn, tree_depth - 1, new_draw_p, new_draw_pos,
                    new_draw_neg, new_mntm_pos, new_mntm_neg, n_p_val, s_p_val,
                    alpha_p_val, n_alpha_p_val, rand_engine, target_data);
    ...
    if (s_p_val == size_t(1)) {
      ...
      if (direction_val == -1) {
        ...
        nuts_build_tree(direction_val, step_size, log_rand_val, prev_U, prev_K,
                        ...
                        new_draw_pp, new_draw_neg, dummy_draw, new_mntm_neg,
                        dummy_mntm, n_pp_val, s_pp_val, alpha_pp_val,
                        n_alpha_pp_val, rand_engine, target_data);
      } else {
        ...
        nuts_build_tree(direction_val, step_size, log_rand_val, prev_U, prev_K,
                        ...
                        box_log_kernel_fn, leap_frog_fn, tree_depth - 1,
                        new_draw_pp, dummy_draw, new_draw_pos, dummy_mntm,
                        new_mntm_pos, n_pp_val, s_pp_val, alpha_pp_val,
                        n_alpha_pp_val, rand_engine, target_data);
      }
      ...
      int check_val_1 = BMO_MATOPS_DOT_PROD(new_draw_pos - new_draw_neg,
                                            new_mntm_neg) >= fp_t(0);
      int check_val_2 = BMO_MATOPS_DOT_PROD(new_draw_pos - new_draw_neg,
                                            new_mntm_pos) >= fp_t(0);
