...
inline vT transform(const vT &vals_inp, const ColVecInt_t &bounds_type,
  ...
  for (size_t i = 0; i < n_vals; ++i) {
    ...
    case 4: // upper and lower bounds
      vals_trans_out(i) = std::log(vals_inp(i) - lower_bounds(i) + eps_dbl) -
                          std::log(upper_bounds(i) - vals_inp(i) + eps_dbl);
      break;
    }
  }
...
template <typename vT>
inline vT
inv_transform(const vT &vals_trans_inp, const ColVecInt_t &bounds_type,
              const ColVec_t &lower_bounds, const ColVec_t &upper_bounds) {
  const size_t n_vals = BMO_MATOPS_SIZE(bounds_type);
  ...
  for (size_t i = 0; i < n_vals; ++i) {
    ...
    case 4: // upper and lower bounds
      if (!std::isfinite(vals_trans_inp(i))) {
        ...
        vals_out(i) =
            (lower_bounds(i) - eps_dbl +
             (upper_bounds(i) + eps_dbl) * std::exp(vals_trans_inp(i))) /
