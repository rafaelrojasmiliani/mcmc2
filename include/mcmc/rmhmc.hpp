...
/**
 * @brief The Riemannian Manifold Hamiltonian Monte Carlo (RM-HMC) MCMC
 * Algorithm
 *
 ...
 *   - \c vals_inp a vector of inputs; and
 *   - \c tensor_deriv_out a 3-dimensional array to store the tensor
 * derivatives; and
 ...
 * draw.
 * @param target_data additional data passed to the user-provided log kernel
 * function.
 * @param tensor_data additional data passed to the user-provided tensor
 * function.
 *
 * @return a boolean value indicating successful completion of the algorithm.
 */

bool rmhmc(const ColVec_t &initial_vals,
           std::function<fp_t(const ColVec_t &vals_inp, ColVec_t *grad_out,
                              void *target_data)>
               target_log_kernel,
           std::function<Mat_t(const ColVec_t &vals_inp,
                               Cube_t *tensor_deriv_out, void *tensor_data)>
               tensor_fn,
